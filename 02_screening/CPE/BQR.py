...
with open("_downlaod_summary.py") as f:
    code = f.read()
    exec(code)
...
summary[['T6M.cum', 'T3M.cum', 'M0.ret', 'N3M.cum', 'N6M.cum']].mean(numeric_only=True)
#################################
# Summary parts: end
#################################

with open("_exit_strategy.py") as f:
    code = f.read()
    exec(code)
#################################
# Strategy: start
...
enter = {
    ...
    'down': -0.1,
    'swing': [-0.1, 0.1],
    'vol_up': 1.5,# 'Trailing.20.vol'
    'vol_down': 0.5# 'Trailing.50.vol'
}
exit = {
    'up': [0.2, -0.2, 150], # [take profit, stop loss, max days]
    'pull.back': [-0.2, 150] # [take profit stop, max days]
}
_dict = {}
#################################
# Entry: asap
cond = close_final['rn']>=enter['asap']
df = close_final[cond].set_index('ticker').copy()
df['rn_trigger'] = enter['asap']
df['cum_ret'] = df.groupby('ticker')['ret']\
    .transform(lambda x: (1 + x).cumprod() - 1)

_dict['asap'] = _exit(df, exit)

#################################
# Entry: down 10%
cond = close_final['cum_ret_d0'] <= enter['down']
df = close_final[cond].groupby('ticker').first().copy()
df = close_final.set_index('ticker').join(df[['cum_ret_d0', 'rn']].add_suffix('_trigger'), how='inner')
df = df[df['rn'] >= df['rn_trigger']].copy()
df['cum_ret'] = df.groupby('ticker')['ret']\
    .transform(lambda x: (1 + x).cumprod() - 1)

_dict['down'] = _exit(df, exit)

#################################
# Entry: swing 10%
cond = (close_final['cum_ret_d0'] <= enter['swing'][0] ) | (close_final['cum_ret_d0'] >= enter['swing'][1])
df = close_final[cond].groupby('ticker').first().copy()
df = close_final.set_index('ticker').join(df[['cum_ret_d0', 'rn']].add_suffix('_trigger'), how='inner')
df = df[df['rn'] >= df['rn_trigger']].copy()
df['cum_ret'] = df.groupby('ticker')['ret']\
...
_dict['swing'] = _exit(df, exit)

#################################
# Entry: vol > 1.5 x Trailing.20.vol
cond = ( close_final['Volume'] / close_final['Trailing.20.vol'] >= enter['vol_up'] )
df = close_final[cond].groupby('ticker').first().copy()
df = close_final.set_index('ticker').join(df[['cum_ret_d0', 'rn']].add_suffix('_trigger'), how='inner')
df = df[df['rn'] >= df['rn_trigger']].copy()
df['cum_ret'] = df.groupby('ticker')['ret']\
...
_dict['Trailing_20_vol'] = _exit(df, exit)

#################################
# Entry: vol < 0.5 x Trailing.50.vol
cond = ( close_final['Volume'] / close_final['Trailing.50.vol'] <= enter['vol_down'] )
df = close_final[cond].groupby('ticker').first().copy()
df = close_final.set_index('ticker').join(df[['cum_ret_d0', 'rn']].add_suffix('_trigger'), how='inner')
df = df[df['rn'] >= df['rn_trigger']].copy()
df['cum_ret'] = df.groupby('ticker')['ret']\
...
_dict['Trailing_50_vol'] = _exit(df, exit)

pd.concat(_dict, axis=1).to_excel('strategy.xlsx')
