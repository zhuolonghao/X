...
df['Avg_wkly_vol_over_3M / Shares Out'] = pd.to_numeric(df['Avg_wkly_vol_over_3M / Shares Out'], errors='coerce')
df['vol_3m / shares'] = df['Avg_wkly_vol_over_3M / Shares Out'].rank(pct=True)
df['Avg_Daily_vol_over_1m / one-year avg daily_vol'] = pd.to_numeric(df['Avg_Daily_vol_over_1m / one-year avg daily_vol'], errors='coerce')
df['vol_1m / 1-yr avg vol'] = df['Avg_Daily_vol_over_1m / one-year avg daily_vol'].rank(pct=True)
df['Avg_Daily_vol_over_3m / one-year avg daily_vol'] = pd.to_numeric(df['Avg_Daily_vol_over_3m / one-year avg daily_vol'], errors='coerce')
