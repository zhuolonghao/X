...
import os
folder = "C:\\Users\\longh\\Desktop\\X\\01_macro_economic\\PCE__QoQ_ShortTerm\\Z.economic_pulse_monthly"
sns.set(style='ticks')
...
for name, lag in {'MoM':1}.items():
    ...
    if lag == 1:
        df.to_excel(f'{folder}\\pce_real.xlsx', index=False)
...
for name, lag in {'MoM':1}.items():
    ...
    df['rank'] = df.groupby('TimePeriod')['pct_chg'].transform(lambda x: x.rank())
    if lag == 1:
        df.to_excel(f'{folder}\\pce.xlsx', index=False)
...
df['LineDescription'] = df['LineDescription'].str[:20]
df['rank'] = df.groupby('TimePeriod')['pct_chg'].transform(lambda x: x.rank())
df.to_excel(f'{folder}\\pce_pi.xlsx', index=False)
...
for name, lag in {'QoQ':1}.items():
    ...
    df['rank'] = df.groupby('TimePeriod')['pct_chg'].transform(lambda x: x.rank())
    if lag == 1:
        df.to_excel(f'{folder}\\pce_real_detail.xlsx', index=False)
...
for name, lag in { 'QoQ':1}.items():
    ...
    df['rank'] = df.groupby('TimePeriod')['pct_chg'].transform(lambda x: x.rank())
    if lag == 1:
        df.to_excel(f'{folder}\\pce_detail.xlsx', index=False)
...
df = pd.concat([df, zero], axis=0)
df['rank'] = df.groupby('TimePeriod')['pct_chg'].transform(lambda x: x.rank())
df.to_excel(f'{folder}\\pce_pi_detail.xlsx', index=False)
